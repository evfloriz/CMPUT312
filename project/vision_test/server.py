...
class Server:
    ...
    # Sends data to the brick via TCP.
    # Input: coords: coordinates of the tracked point
    #        queue [Thread-safe Queue]: Mutable data structure to store (and return) the messages received from the client
    def sendCoords(self, coords, queue):
        # Format in which the client expects the data: x, y, r
        data = str(coords[0]) + "," + str(coords[1]) + "," + str(coords[2])
